...
#define SetControlPosAlignBottomLeft(a, b)							\
	SetWindowPos(GetDlgItem(hSelDlg, a), hSelDlg, b[0], b[1] - yDelta, b[2] - xDelta, b[3], SWP_NOZORDER | SWP_NOSIZE | SWP_NOSENDCHANGING);
// 底左水平拉伸
#define SetControlPosAlignBottomLeftResizeHor(a, b)				\
	SetWindowPos(GetDlgItem(hSelDlg, a), hSelDlg, b[0], b[1] - yDelta, b[2] - xDelta - xScrollBarDelta, b[3], SWP_NOZORDER | SWP_NOSENDCHANGING);
// 底左水平拉伸
#define SetControlPosAlignBottomLeftResizeHor1(a, b)				\
	SetWindowPos(GetDlgItem(hSelDlg, a), hSelDlg, b[0], b[1] - yDelta, b[2] - xDelta - xScrollBarDelta, b[3], SWP_NOZORDER | SWP_NOSIZE | SWP_NOSENDCHANGING);
// 右上垂直拉伸
#define SetControlPosAlignTopRightResizeVert(a, b)				\
	xScrollBarDelta = (a == IDC_TREE2) ? -18 : 0;				\
	SetWindowPos(GetDlgItem(hSelDlg, a), hSelDlg, b[0], b[1], b[2] - xScrollBarDelta, b[3] - yDelta, SWP_NOZORDER | SWP_NOSENDCHANGING);
// 底右垂直拉伸
#define SetControlPosAlignTopRightResizeVert9(a, b)				\
	xScrollBarDelta = (a == IDC_TREEWEIJIA) ? -18 : 0;				\
	SetWindowPos(GetDlgItem(hSelDlg, a), hSelDlg, b[0] - xDelta, b[1], b[2], b[3] - yDelta, SWP_NOZORDER | SWP_NOSENDCHANGING);
...
//		SendDlgItemMessage(hDlg, IDC_SCREENSHOT2_V, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)NULL);
	...
	if (Msg == WM_SIZE) {
		// Redrawing is allowed when changing the size
...
//		SetControlPosAlignTopLeft(IDC_STATIC2, nDlgPreviewGrpInitialPos);
		...
		SetControlPosAlignTopLeft(IDC_SEL_TAB, nDlgSelTabInitialPos);
		SetControlPosAlignTopLeft(IDC_SEL_APPLYIPS, nDlgApplyIpsChbInitialPos);
		SetControlPosAlignTopLeft(IDC_SEL_IPSMANAGER, nDlgIpsManBtnInitialPos);
		
		SetControlPosAlignTopLeft(IDC_STATIC_OPT, nDlgOptionsGrpInitialPos);
		SetControlPosAlignTopLeft(IDC_CHECKAVAILABLE, nDlgAvailableChbInitialPos);
		SetControlPosAlignTopLeft(IDC_CHECKUNAVAILABLE, nDlgUnavailableChbInitialPos);
		SetControlPosAlignTopLeft(IDC_CHECKAUTOEXPAND, nDlgAlwaysClonesChbInitialPos);
		SetControlPosAlignTopLeft(IDC_SEL_SHORTNAME, nDlgZipnamesChbInitialPos);
		SetControlPosAlignTopLeft(IDC_SEL_ASCIIONLY, nDlgLatinTextChbInitialPos);
		SetControlPosAlignTopLeft(IDC_SEL_SUBDIRS, nDlgSearchSubDirsChbInitialPos);
		SetControlPosAlignTopLeft(IDC_SEL_DISABLECRC, nDlgDisableCrcInitialPos);
		SetControlPosAlignTopLeft(IDC_SEL_APPLYIPS, nDlgApplyIpsChbInitialPos);
		SetControlPosAlignTopLeft(IDC_SEL_IPSMANAGER, nDlgIpsManBtnInitialPos);	
		SetControlPosAlignTopLeft(IDROM, nDlgRomDirsBtnInitialPos);
		SetControlPosAlignTopLeft(IDRESCAN, nDlgScanRomsBtnInitialPos);
...
//		SetControlPosAlignBottomLeft(IDC_SEL_IPSGROUP, nDlgIpsGrpInitialPos);

		SetControlPosAlignBottomLeft(IDC_SEL_SEARCHGROUP, nDlgSearchGrpInitialPos);
		SetControlPosAlignBottomLeft(IDC_SEL_SEARCH, nDlgSearchTxtInitialPos);
		SetControlPosAlignBottomLeft(IDCANCEL, nDlgCancelBtnInitialPos);
		SetControlPosAlignBottomLeft(IDOK, nDlgPlayBtnInitialPos);

		SetControlPosAlignBottomLeft(IDC_STATIC_INFOBOX, nDlgWhiteBoxInitialPos);
		SetControlPosAlignBottomLeft(IDC_LABELCOMMENT, nDlgGameInfoLblInitialPos);
		SetControlPosAlignBottomLeft(IDC_LABELROMNAME, nDlgRomNameLblInitialPos);
		SetControlPosAlignBottomLeft(IDC_LABELROMINFO, nDlgRomInfoLblInitialPos);
		SetControlPosAlignBottomLeft(IDC_LABELSYSTEM, nDlgReleasedByLblInitialPos);
		SetControlPosAlignBottomLeft(IDC_LABELGENRE, nDlgGenreLblInitialPos);
		SetControlPosAlignBottomLeft(IDC_LABELNOTES, nDlgNotesLblInitialPos);
		SetControlPosAlignBottomLeft(IDC_TEXTCOMMENT, nDlgGameInfoTxtInitialPos);
		SetControlPosAlignBottomLeft(IDC_TEXTROMNAME, nDlgRomNameTxtInitialPos);
		SetControlPosAlignBottomLeft(IDC_TEXTROMINFO, nDlgRomInfoTxtInitialPos);
		SetControlPosAlignBottomLeft(IDC_TEXTSYSTEM, nDlgReleasedByTxtInitialPos);
		SetControlPosAlignBottomLeft(IDC_TEXTGENRE, nDlgGenreTxtInitialPos);
		SetControlPosAlignBottomLeft(IDC_TEXTNOTES, nDlgNotesTxtInitialPos);
		SetControlPosAlignBottomLeft(IDC_DRVCOUNT, nDlgDrvCountTxtInitialPos);
		SetControlPosAlignTopLeft(IDGAMEINFO, nDlgDrvRomInfoBtnInitialPos);	
...
//					dprintf(_T("  - Item (%i?i) - (%i?i) %hs\n"), lplvcd->nmcd.rc.left, lplvcd->nmcd.rc.top, lplvcd->nmcd.rc.right, lplvcd->nmcd.rc.bottom, ((NODEINFO*)TvItem.lParam)->pszROMName);
					...
					if (!(TvItem.state & (TVIS_SELECTED | TVIS_DROPHILITED))) {
						...
						if (!((NODEINFO*)TvItem.lParam)->bIsParent) {
							lplvcd->clrTextBk = RGB(0xF5, 0xF5, 0xF5);
