...
struct RomFind { INT32 nState; INT32 nZip; INT32 nPos; INT32 nLen;};				// State is non-zero if found. 1 = found totally okay.
...
static INT32 __cdecl BzipBurnLoadRom(UINT8* Dest, INT32* pnWrote, INT32 i)
{
	...
	if (ri.nLen < RomFind[i].nLen) {
		ri.nLen = RomFind[i].nLen;
	}
		...
		if (ZipOpen(TCHARToANSI(pszBzipName, NULL, 0)) == 0) {		// Open the rom zip file
			...
			for (INT32 i = 0; i < nRomCount; i++) {
				...
				if (List[nFind].nLen == ri.nLen) {
					...
					if (nLoadMenuShowY & (1 << 25)) {
						ri.nLen = List[nFind].nLen;						// Diasble size check
						RomFind[i].nLen = List[nFind].nLen;				// 扩容部分可以正常读取
					} else {
					if (List[nFind].nLen < ri.nLen) {
