...
//			VidRedraw();
		...
		case MENU_CONTENTS: {
			if (!nVidFullscreen) {
				FILE* fp = _tfopen(_T("support\\出招表.exe"), _T("r"));
				if (fp) {
					fclose(fp);
					ShellExecute(NULL, _T("open"), _T("support\\出招表.exe"), NULL, NULL, SW_SHOWNORMAL);
				}
			}
			break;
		}
		...
		case MENU_WHATSNEW: {
			if (!nVidFullscreen) {
				FILE* fp = _tfopen(_T("support\\出招表.chm"), _T("r"));
				if (fp) {
					fclose(fp);
					ShellExecute(NULL, _T("open"), _T("support\\出招表.chm"), NULL, NULL, SW_SHOWNORMAL);
				}
			}
			break;
		}
		...
		case MENU_WWW_HOME:
			if (!nVidFullscreen) {
				FILE* fp = _tfopen(_T("support\\fbneo使用教程.chm"), _T("r"));
				if (fp) {
					fclose(fp);
					ShellExecute(NULL, _T("open"), _T("support\\fbneo使用教程.chm"), NULL, NULL, SW_SHOWNORMAL);
				}
			}
			break;
		...
		case MENU_WWW_NSFORUM:
			if (!nVidFullscreen) {
				ShellExecute(NULL, _T("open"), _T("https://jq.qq.com/?_wv=1027&k=uw9RekLh"), NULL, NULL, SW_SHOWNORMAL);
			}
			break;

		case MENU_WWW_GITHUB:
			if (!nVidFullscreen) {
				ShellExecute(NULL, _T("open"), _T("https://www.ppxclub.com/forum.php?mod=viewthread&tid=703500&page=1&extra=#pid8467528"), NULL, NULL, SW_SHOWNORMAL);
			}
			break;
